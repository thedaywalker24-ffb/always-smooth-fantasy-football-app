...
5. Verify the Apps Script URL still serves the relevant read routes, usually `api=config`, `api=league-data`, `api=ticker-data`, `api=draft-board`, and `api=matchups-data`; use `debug=1` when diagnostics are useful.
