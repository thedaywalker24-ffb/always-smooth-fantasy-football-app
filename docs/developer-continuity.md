...
* The Home ticker route `api=ticker-data` calls Rotoworld/NBC fantasy football RSS for headlines and ESPN public NFL scoreboard for live scores from Apps Script. `getTickerItems()` uses `new Date().getMonth()` to switch modes: Feb-Aug headlines only, Sep-Jan two score items followed by one headline where available.
