...
* The Matchups tab reads from `All Matchups`, not directly from Sleeper. Required display headers are `Matchup ID`, a team-name column such as `Team Name`, `Photo`, `Record`, and `Week Points`; manager and roster ID columns are optional. Groups with anything other than exactly two teams are excluded so playoff/offseason partial matchup data does not show orphan teams.
