...
* Betting leader chart is frontend-only once `seasonBetsWon` exists in `api=betting-data`; it sorts members by numeric season total, takes the top five, and hides if every total is zero/blank.
