...
* The four tour targets are `#announcement-tile` (fallback `#ticker-section`), `.owner-tile--mine` (fallback `#standings-grid`), `[data-jump-to-draft]` (fallback `#draft-board-section`), and `#tab-betting` (fallback `#betting-panel`). Preserve these hooks or update `APP_TOUR_STEPS`. Home and Betting expose `[data-tour-replay]`.
