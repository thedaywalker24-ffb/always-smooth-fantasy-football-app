...
* Weekly Captain data uses `Team Rosters` current `Starter` rows for eligible players and enriches position/NFL team from `Sleeper Players`. Captain player photos are derived from Sleeper player IDs. `Weekly Captains` row 1 is repaired on submit and stores one auditable row per `Season + Week + Team Name`.
