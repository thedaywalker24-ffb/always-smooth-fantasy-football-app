...
* The Home ticker route `api=ticker-data` calls Rotoworld/NBC fantasy football sources for headlines and ESPN public NFL scoreboard for live scores from Apps Script. `getTickerItems()` uses `new Date().getMonth()` to switch modes: Feb-Aug headlines only, Sep-Jan two score items followed by one headline where available. The Rotoworld parser tries the requested RSS URL, the page Atom feed, then the server-rendered player-news page because the RSS/Atom sources can be empty or unavailable.
