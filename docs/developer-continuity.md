...
* The Home ticker route `api=ticker-data` calls ESPN public NFL news and scoreboard endpoints from Apps Script. `getTickerItems()` uses `new Date().getMonth()` to switch modes: Feb-Aug headlines only, Sep-Jan two score items followed by one headline where available.
